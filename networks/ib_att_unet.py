...
class IB_Attention_UNet_k5(nn.Module):
    ...
    def __init__(self, opt, is_deconv=True, nonlocal_mode='concatenation', attention_dsample=(2,2,2), is_batchnorm=True):
        ...
        self.conv_On_filters = IB_filters_3D(radius=2.0, gamma=2. / 3., in_channels=n_kernels, 
                                                    out_channels=n_kernels, off=False).to(self.device)
        self.conv_Off_filters = IB_filters_3D(radius=2.0, gamma=2. / 3., in_channels=n_kernels, 
...
class IB_Attention_UNet_k3(nn.Module):
    ...
    def __init__(self, opt, is_deconv=True, nonlocal_mode='concatenation', attention_dsample=(2,2,2), is_batchnorm=True):
        ...
        self.conv_On_filters = IB_filters_3D(radius=1.0, gamma=1. / 2., in_channels=n_kernels, 
                                                    out_channels=n_kernels, off=False).to(self.device)
        self.conv_Off_filters = IB_filters_3D(radius=1.0, gamma=1. / 2., in_channels=n_kernels, 
                                                    out_channels=n_kernels, off=True).to(self.device)
