...
class IB_UNet_k5(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.conv_On_filters = IB_filters_3D(radius=2.0, gamma=2. / 3., in_channels=self.n_kernels*2, 
                                                    out_channels=self.n_kernels*2, off=False).to(self.device)
        self.conv_Off_filters = IB_filters_3D(radius=2.0, gamma=2. / 3., in_channels=self.n_kernels*2, 
        ...
        self.enc1 = IB_Block(in_channels=self.n_kernels*2, out_channels=self.n_kernels*2, nonlin=self.nonlin, downconv=False, dropout_rate=self.dropout_rate)
...
class IB_UNet_k3(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.conv_On_filters = IB_filters_3D(radius=1.0, gamma=1. / 2., in_channels=self.n_kernels*2, 
                                                    out_channels=self.n_kernels*2, off=False).to(self.device)
        self.conv_Off_filters = IB_filters_3D(radius=1.0, gamma=1. / 2., in_channels=self.n_kernels*2, 
                                                    out_channels=self.n_kernels*2, off=True).to(self.device)
