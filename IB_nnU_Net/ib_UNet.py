...
class IBConvLayers(nn.Module):
    def __init__(self, input_feature_channels, output_feature_channels, num_convs,
        ...
        super(IBConvLayers, self).__init__()
        self.conv1 = basic_block(input_feature_channels, int(output_feature_channels/2), self.conv_op,
                           self.conv_kwargs_first_conv,
                           self.norm_op, self.norm_op_kwargs, self.dropout_op, self.dropout_op_kwargs,
                           self.nonlin, self.nonlin_kwargs)

        self.conv2 = basic_block(input_feature_channels, int(output_feature_channels/2), self.conv_op,
                           self.conv_kwargs_first_conv,
                           self.norm_op, self.norm_op_kwargs, self.dropout_op, self.dropout_op_kwargs,
                           self.nonlin, self.nonlin_kwargs)         

        self.conv_On_filters = IB_filters_3D(radius=2.0, gamma=2. / 3., 
                                             in_channels=input_feature_channels, 
                                             out_channels=int(output_feature_channels/2), 
                                             off=False)
        self.conv_Off_filters = IB_filters_3D(radius=2.0, gamma=2. / 3., 
                                              in_channels=input_feature_channels, 
                                              out_channels=int(output_feature_channels/2), 
                                              off=True)
        ...
        self.conv3 = basic_block(int(output_feature_channels/2), int(output_feature_channels/2), self.conv_op,
                           self.conv_kwargs,
                           self.norm_op, self.norm_op_kwargs, self.dropout_op, self.dropout_op_kwargs,
                           self.nonlin, self.nonlin_kwargs)

        self.conv4 = basic_block(int(output_feature_channels/2), int(output_feature_channels/2), self.conv_op,
                           self.conv_kwargs,
                           self.norm_op, self.norm_op_kwargs, self.dropout_op, self.dropout_op_kwargs,
                           self.nonlin, self.nonlin_kwargs)
    ...
    def forward(self, enc_1_skip, enc_1_pool):
        output1 = self.conv1(enc_1_pool)
        output2 = self.conv2(enc_1_pool)

        # On and Off surround modulation
        sm_on = self.lrelu(self.surround_modulation_DoG_on(enc_1_skip))
        sm_off = self.lrelu(self.surround_modulation_DoG_off(enc_1_skip))
    ...
    def surround_modulation_DoG_on(self, input):
        return F.conv3d(input, weight=self.conv_On_filters, stride=tuple(self.ib_strides), padding=(2,2,2))

    def surround_modulation_DoG_off(self, input):
        return F.conv3d(input, weight=self.conv_Off_filters, stride=tuple(self.ib_strides), padding=(2,2,2))
...
class IB_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for d in range(num_pool):
            ...
            # add convolutions
            if d == 1:
                self.conv_blocks_context.append(IBConvLayers(input_features, output_features, num_conv_per_stage,
                                                                self.conv_op, self.conv_kwargs, self.norm_op,
                                                                self.norm_op_kwargs, self.dropout_op,
                                                                self.dropout_op_kwargs, self.nonlin, self.nonlin_kwargs,
                                                                first_stride, basic_block=basic_block,
                                                                ib_strides=pool_op_kernel_sizes[d - 1]))
            else:
    ...
    def forward(self, x):
        ...
        for d in range(len(self.conv_blocks_context) - 1):
            ...
            if d == 0:
                ...
                skips.append(x)
                if not self.convolutional_pooling:
                    pool_out = self.td[d](x)

            elif d ==1:
                x = self.conv_blocks_context[d](x, pool_out)
                skips.append(x)
                if not self.convolutional_pooling:
                    x = self.td[d](x)
